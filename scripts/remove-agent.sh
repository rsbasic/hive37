...
echo "Run these commands on this machine to finish cleanup:"
echo ""
echo "1. Kill the gateway process (if still running):"
echo "   kill \$(lsof -ti :PORT_NUMBER)"
echo ""
echo "2. Remove Syncthing completely:"
echo "   launchctl unload ~/Library/LaunchAgents/syncthing.plist 2>/dev/null; pkill syncthing; rm -rf ~/Library/Application\\ Support/Syncthing/; rm -f ~/Library/LaunchAgents/syncthing.plist; rm -rf ~/conclave-sync/; rm -rf ~/hive37/; brew uninstall syncthing 2>/dev/null; echo 'Syncthing removed'"
echo ""
echo "3. Ask a hive admin to kick $AGENT_ID from Matrix rooms"
echo "   (if the script couldn't find an admin token)"
echo ""
echo "4. Rotate shared API keys on the admin machine"
